	...
	switch($_POST['action']){
		case 'signup':
			...
			if(count($result)==0){//name not taken, register and reload with name in url
				...
				header("Location: game1.php?playername=$name");
				return;
		...
		case 'notifyentergame':
			...
			cout($query);
			/*
			$query="SELECT * FROM player JOIN $lobby ON player.name=$lobby.playername ";
			$result=$conn->query($query) or die($conn->error);
			cout(json_encode($result->fetch_all(MYSQLI_ASSOC)));
			cout($query);*/
	...
	switch($page){
		...
		case 'browser':
			...
			foreach($result as $lobby){
				...
				<form method="post" action="game1.php?playername=<?PHP echo $playername?>">
